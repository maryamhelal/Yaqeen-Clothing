...
import { faCartShopping, faBars, faUser, faTimes } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <>
      <nav className="bg-white shadow-lg border-b border-gray-100 relative z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              <FontAwesomeIcon icon={menuOpen ? faTimes : faBars} className="w-7 h-7 transition-all duration-300" />
            </button>
          </div>
        </div>
      </nav>
      ...
      <div className={`fixed inset-0 z-40 transition-all duration-300 ${menuOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        {/* Overlay */}
        <div
          className={`fixed inset-0 bg-black bg-opacity-30 transition-opacity duration-300 ${menuOpen ? 'opacity-100' : 'opacity-0'}`}
          onClick={() => setMenuOpen(false)}
          aria-hidden="true"
        />
        {/* Drawer */}
        <div className={`fixed top-0 left-0 h-full w-72 bg-white shadow-xl z-50 transform transition-transform duration-300 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
